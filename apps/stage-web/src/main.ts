...
const fullAvatar = `${import.meta.env.BASE_URL}recomposite.png`
  ...
  <main class="shell" data-emotion="neutral">
    ...
    <section class="stage" id="stage" aria-label="NIVA full-frame stage">
      ...
      <div class="avatar-shell" id="avatar">
        <img id="avatarFrame" class="avatar-frame" src="${fullAvatar}" alt="NIVA" draggable="false" />
      </div>
...
function typeText(text:string){
  ...
  typeTimer=window.setInterval(()=>{speech.textContent=text.slice(0,++i); if(i>=text.length) clearInterval(typeTimer)},22)
...
function setEmotion(emotion:Expression,intensity=.8){
  shell.dataset.emotion=emotion; shell.style.setProperty('--emotion-intensity',String(Math.max(0,Math.min(1,intensity))))
...
function playMotion(motion?:MotionName){
  if(!motion)return; clearTimeout(motionTimer)
  avatar.classList.remove(...Array.from(avatar.classList).filter(c=>c.startsWith('motion-'))); void avatar.offsetWidth; avatar.classList.add(`motion-${motion}`)
  document.querySelectorAll<HTMLButtonElement>('[data-motion]').forEach(b=>b.classList.toggle('active',b.dataset.motion===motion))
  motionTimer=window.setTimeout(()=>{avatar.classList.remove(`motion-${motion}`); document.querySelectorAll<HTMLButtonElement>('[data-motion]').forEach(b=>b.classList.remove('active'))},1500)
}
function act(action:NivaAction){
  lastInteraction=Date.now(); if(action.emotion)setEmotion(action.emotion,action.expressionIntensity??.8); if(action.motion)playMotion(action.motion); if(action.text)typeText(action.text); scheduleIdle()
}
function replyFor(text:string):NivaAction{
  const t=text.trim(), l=t.toLowerCase(); if(!t)return{text:'嗯？你可以直接和我说。',emotion:'happy',motion:'greet'}
...
function sendMessage(text:string){if(!text.trim())return; input.value=''; act({text:'让我想一下…',emotion:'thinking',motion:'thinking'}); setTimeout(()=>act(replyFor(text)),420)}
function addButtons<T extends string>(id:string,values:Array<[T,string]>,attr:string,fn:(v:T)=>void){const host=document.querySelector<HTMLElement>(`#${id}`)!; for(const [v,label] of values){const b=document.createElement('button'); b.textContent=label; b.dataset[attr]=v; b.onclick=()=>fn(v); host.appendChild(b)}}
addButtons('expressions',expressions,'emotion',setEmotion); addButtons('motions',motions,'motion',playMotion)

function scheduleIdle(){clearTimeout(idleTimer); idleTimer=window.setTimeout(()=>{if(Date.now()-lastInteraction<12000)return scheduleIdle(); const pool:NivaAction[]=[{text:'我还在这里。',emotion:'neutral',motion:'lookAround'},{text:'刚刚安静了一会儿，我在等你。',emotion:'shy',motion:'greet'}]; act(pool[Math.floor(Math.random()*pool.length)])},15000)}
scheduleIdle()
...
Object.assign(window,{NIVA:{act,setEmotion,motion:playMotion,send:sendMessage,get ready(){return true},get mode(){return '2d-full-frame' as const}}})
declare global{interface Window{NIVA:{act(action:NivaAction):void;setEmotion(emotion:Expression,intensity?:number):void;motion(motion?:MotionName):void;send(text:string):void;readonly ready:boolean;readonly mode:'2d-full-frame'}}}
setEmotion('neutral'); setTimeout(()=>act({text:'你好，我已经醒了。想和我聊点什么？',emotion:'happy',motion:'greet'}),300)
