...
const life={
  ...
  update(now){
    ...
    if(settings.blinkEnabled&&vrm.expressionManager){
      ...
      if(this.blinkStart){const t=now-this.blinkStart;let v=t<90?t/90:t<145?1:(t<275?1-(t-145)/130:0);vrm.expressionManager.setValue('blink',clamp(v,0,1));if(t>300){this.blinkStart=0;this.nextBlink=now+3200+Math.random()*2600;if(this.blinkDouble)this.nextBlink=now+180;}}
    }
    if(settings.breathingEnabled){ const breathHz=settings.breaths/60; const amp=(now<this.deepBreathUntil?.valueOf()?settings.breathAmp*1.8:settings.breathAmp); const breath=Math.sin((now/1000)*Math.PI*2*breathHz)*amp; applyAdditive('chest',breath*1.1,0,0); applyAdditive('upperChest',breath*.75,0,0); applyAdditive('spine',breath*.28,0,0); }
    if(settings.heartbeatEnabled){ const beatPhase=((now/1000)*(settings.bpm/60))%1; const pulse=(Math.exp(-Math.pow((beatPhase-.06)/.035,2))*.18+Math.exp(-Math.pow((beatPhase-.22)/.05,2))*.08); applyAdditive('upperChest',pulse,0,0,'heartbeat'); }
  }
};
...
function applyManualAndLife(){
  ...
  for(const [name,base] of baseQuats.entries()){
    if(currentAction && ['walk','run','wave','think','reach','weight','nod'].includes(currentActionName) && !manualOffsets.has(name)) continue;
...
function renderExpressionControls(){
  ...
  controlPage.innerHTML=`<section class="panel-section"><h3>预设表情</h3><div class="button-grid">${names.map(([l,n])=>`<button data-exp="${n}" class="secondary-btn">${l}</button>`).join('')}</div>${rowSlider('表情强度',0,1,.05,.7)}<button id="clearExp" class="secondary-btn wide">清除表情</button></section>`;const slider=controlPage.querySelector('input[type=range]');controlPage.querySelectorAll('[data-exp]').forEach(b=>b.onclick=()=>setExpression(b.dataset.exp,Number(slider.value),220));$('#clearExp').onclick=()=>setExpression('neutral',0,180);
}
...
function applyLightPreset(p){const m={natural:[.32,1.15,.42,.55,0,.9],soft:[.42,.85,.55,.35,0,.9],warm:[.28,1.25,.35,.5,0,.88],cool:[.25,1.0,.6,.75,0,.88],stage:[.18,.8,.25,.8,2.2,.82]}[p];if(!m)return;[settings.ambient,settings.key,settings.fill,settings.rim,settings.stageLight,settings.exposure]=m;key.color.set(p==='warm'?0xffddc7:0xffffff);fill.color.set(p==='cool'?0xa6d9ff:0xc8ddff);applyLighting();saveSettings();renderLightControls();}
function renderCameraControls(){controlPage.innerHTML=`<section class="panel-section"><h3>相机</h3><div class="button-grid"><button data-cam="full">全身</button><button data-cam="upper">半身</button><button data-cam="face">脸部</button><button data-cam="left">左侧</button><button data-cam="right">右侧</button><button data-cam="back">背面</button></div>${rowSlider('FOV',18,55,1,camera.fov)}${rowSlider('旋转速度',.2,1.5,.1,controls.rotateSpeed)}${rowSlider('缩放速度',.2,1.5,.1,controls.zoomSpeed)}</section>`;controlPage.querySelectorAll('[data-cam]').forEach(b=>b.onclick=()=>cameraPreset(b.dataset.cam));const s=controlPage.querySelectorAll('input[type=range]');s[0].oninput=()=>{camera.fov=Number(s[0].value);camera.updateProjectionMatrix();s[0].parentElement.querySelector('output').textContent=s[0].value;};s[1].oninput=()=>{controls.rotateSpeed=Number(s[1].value);s[1].parentElement.querySelector('output').textContent=s[1].value;};s[2].oninput=()=>{controls.zoomSpeed=Number(s[2].value);s[2].parentElement.querySelector('output').textContent=s[2].value;};}
...
$('#panelToggle').onclick=()=>{panel.classList.toggle('hidden');upgradePanel.classList.add('hidden');settings.panelVisible=!panel.classList.contains('hidden');saveSettings();renderControl();};$('#closePanel').onclick=()=>{panel.classList.add('hidden');settings.panelVisible=false;saveSettings();};$('#upgradeToggle').onclick=()=>{upgradePanel.classList.toggle('hidden');panel.classList.add('hidden');};$('#closeUpgrade').onclick=()=>upgradePanel.classList.add('hidden');
