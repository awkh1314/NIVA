...
function applyManualAndLife(){
  ...
  for(const [name,base] of baseQuats.entries()){
    if(currentAction && ['walk','run','wave','think','thinkLoop','reach','weight','nod'].includes(currentActionName) && !manualOffsets.has(name)) continue;
...
window.NIVA={version:'0.93-free-life',speak,act:(name)=>performAction(name),play:(name)=>playClip(name,{duration:clips.get(name)?.duration||2}),stop:stopAction,state:()=>({modelReady,speaking,currentAction:currentActionName,persistentPreview,director:director.state})};
