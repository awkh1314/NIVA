...
export function sampleDayNight(timeOfDay=0.25){
  ...
  const top=new THREE.Color().setRGB(THREE.MathUtils.lerp(.025,.25,dayFactor),THREE.MathUtils.lerp(.045,.56,dayFactor),THREE.MathUtils.lerp(.10,.91,dayFactor));
  const horizonDay=new THREE.Color(0xc9e5f1),horizonNight=new THREE.Color(0x18243a),sunset=new THREE.Color(0xf2a06f);const horizon=horizonNight.clone().lerp(horizonDay,dayFactor).lerp(sunset,horizonFactor*(.86-.42*dayFactor));
  const ground=new THREE.Color(0x101820).lerp(new THREE.Color(0x91b8a0),dayFactor*.75);const sunColor=new THREE.Color(0xff9b63).lerp(new THREE.Color(0xfff3d6),clamp(Math.abs(sunY)*1.7,0,1));const moonColor=new THREE.Color(0xaec8ff);
...
export function hourToCycle(hour=12){return (((Number(hour)||0)-6)/24%1+1)%1;}
export function cycleToHour(timeOfDay=0){return (((Number(timeOfDay)||0)*24+6)%24+24)%24;}
...
export class DayNightCycle{
  ...
  setHour(hour){return this.setTimeOfDay(hourToCycle(hour));}
  ...
  state(){return {timeOfDay:this.timeOfDay,hour:cycleToHour(this.timeOfDay),auto:this.auto,speed:this.speed,dayFactor:this.last.dayFactor,nightFactor:this.last.nightFactor,starOpacity:this.last.starOpacity};}
}
