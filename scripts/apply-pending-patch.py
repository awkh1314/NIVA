...
physics = Path('src/runtime/physics/niva-body-physics.mjs')
s = physics.read_text(encoding='utf-8')

if 'structured-squat-v4-flat-feet' not in s:
    s = s.replace(
    ...
    old = '''  setPosture(action, dt, crouchDepth = 0.19) {
...
'''
    new = '''  setPosture(action, dt, crouchDepth = 0.19) {
...
'''
    if old not in s:
        raise SystemExit('setPosture block not found')
    s = s.replace(old, new, 1)

    old = "    this.setPosture(action, dt, crouchDepth);\n    this.vrm.scene.updateMatrixWorld(true);\n"
    new = "    const crouchAmount = this.setPosture(action, dt, crouchDepth);\n    if (action === 'crouch') this.applyCrouchBalance(crouchAmount);\n    this.vrm.scene.updateMatrixWorld(true);\n"
    if old not in s:
        raise SystemExit('posture call marker not found')
    s = s.replace(old, new, 1)

    old = "        if (stance && this.footAnchor[side]) this.solveLeg(side, this.footAnchor[side], this.ikStrength, action);\n      }\n    }\n\n    // Upper-body action IK is independent from the foot-IK toggle.\n"
    new = "        if (stance && this.footAnchor[side]) this.solveLeg(side, this.footAnchor[side], this.ikStrength, action);\n      }\n      if (action === 'crouch') this.stabilizeCrouchFeet(crouchAmount);\n    }\n\n    // Upper-body action IK is independent from the foot-IK toggle.\n"
    if old not in s:
        raise SystemExit('foot stabilization insertion marker not found')
    s = s.replace(old, new, 1)
    ...
    old_leg = '''    const { right, forward } = this.bodyBasis();
...
'''
    new_leg = '''    const { right, forward } = this.bodyBasis();
    const sign = side === 'left' ? -1 : 1;
    const hip = upper.getWorldPosition(new THREE.Vector3());
...
'''
    if old_leg not in s:
        raise SystemExit('solveLeg marker not found')
    s = s.replace(old_leg, new_leg, 1)

    marker = "  solveCrouchHandsToHead(weight = 0.96) {\n"
    methods = '''  rotateWorldAround(bone, axis, radians, weight = 1) {
...
'''
    if marker not in s:
        raise SystemExit('crouch hands marker not found')
...
physics.write_text(s, encoding='utf-8')

main = Path('src/main.js')
m = main.read_text(encoding='utf-8')
pattern = r"(?m)^\s*clips\.set\('crouch'.*$"
replacement = "  clips.set('crouch',makeClip('crouch',2,{leftUpperLeg:[[0,2,0,0],[2,2,0,0]],rightUpperLeg:[[0,2,0,0],[2,2,0,0]],leftLowerLeg:[[0,6,0,0],[2,6,0,0]],rightLowerLeg:[[0,6,0,0],[2,6,0,0]],leftFoot:[[0,0,0,0],[2,0,0,0]],rightFoot:[[0,0,0,0],[2,0,0,0]],spine:[[0,0,0,0],[2,0,0,0]],chest:[[0,0,0,0],[2,0,0,0]],head:[[0,0,0,0],[2,0,0,0]]}));"
m, n = re.subn(pattern, replacement, m, count=1)
if n != 1:
    raise SystemExit('main crouch clip line not found')
m = m.replace("crouchDepth:0.19,", "crouchDepth:0.20,", 1)
m = m.replace("version:'0.95-biomechanics-v3'", "version:'0.96-structured-squat-v4'", 1)
main.write_text(m, encoding='utf-8')

Path('DEPLOY_VERSION.txt').write_text(
    'NIVA Biomechanics V4\n'
    'Structured squat: corrected anatomical front/back; synchronized pelvis drop + forward torso balance + knee bend; feet/toes stay flat; root Y compensates from real foot-anchor error; hands remain on head.\n',
    encoding='utf-8',
)

for stale in [
    Path('.github/workflows/hotfix-runtime.yml'),
    Path('.github/workflows/fix-crouch-structured-v4.yml'),
]:
    if stale.exists():
        stale.unlink()
